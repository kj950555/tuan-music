      ...
      <van-tabs v-model="active">
        <van-tab v-for="(imen,index) in correlation" :key="index">
          ...
          <van-cell :sticky="true" is-link v-for="(item , i) in imen.content" :key="i" >
            ...
            <template #right-icon>
              <i class="iconfont icon-play" @click="PlayBack(item)" >&#xe618;</i>
            </template>
          </van-cell>
        </van-tab>
      </van-tabs>
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      vm.axios({
        ...
        .then((res) => {
          console.log(res);
          vm.correlation[0].content = res.data.songs;
          console.log(vm.correlation);
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      vm.axios({
        methods: "GET",
        url: `/artist/album?id=${vm.SingerInformation.id}`,
      })
        .then((res) => {
          console.log("专辑==》", res);
          vm.correlation[1].content = res.data.hotAlbums;
        })
        .catch((err) => {
          console.log(err);
        });
